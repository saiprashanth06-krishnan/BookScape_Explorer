...
def fetch_books(search_key, max_results=40):
    all_books = []
    start_index = 0  # Start from the first result
    total_items = None  # To track the total number of books available
    
    while True:
        url = f"https://www.googleapis.com/books/v1/volumes?q={search_key}&startIndex={start_index}&maxResults={max_results}"
        response = requests.get(url)
        data = response.json()
        
        if total_items is None:
            total_items = data.get("totalItems", 0)  # Get total available books

        books = data.get("items", [])
        if not books:
            break  # Stop when no more books are found

        all_books.extend(books)  # Add books to list
        start_index += max_results  # Move to the next page

        if start_index >= total_items:  
            break  # Stop if all books are fetched
    
    return {"items": all_books}  # Return all books
...
# Function to fetch stored books from the database based on search key
def get_stored_books(search_key):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""SELECT book_id, search_key, book_title, book_subtitle, book_authors, book_description,
                              industryIdentifiers, text_readingModes, image_readingModes, pageCount, categories,
                              language, imageLinks, ratingsCount, averageRating, country, saleability, isEbook,
                              amount_listPrice, currencyCode_listPrice, amount_retailPrice, currencyCode_retailPrice,
                              buyLink, year, publisher FROM books WHERE search_key = %s""", (search_key,))
...
if st.sidebar.button("Search & Store"):
    if search_key:
        books = fetch_books(search_key)  # Fetch all books with pagination
        stored_count = store_books(books, search_key)
        st.sidebar.success(f"Books stored successfully! Total books stored: {stored_count}")
    else:
        st.sidebar.error("Please enter a search key.")


# Display stored books for the entered search key
if search_key:
    st.header(f"📖 Books related to '{search_key}'")
    stored_books = get_stored_books(search_key)
    if stored_books:
        df = pd.DataFrame(stored_books)
        st.dataframe(df)
    else:
        st.write("No books found for this search key.")    
...
queries = {
    ...
    "Top 5 Most Expensive Books": "SELECT book_title, pageCount as price FROM books ORDER BY pageCount DESC LIMIT 5;",
